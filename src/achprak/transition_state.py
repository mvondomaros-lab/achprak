...
class OptTS:
    ...
    def run(self, output=None, steps=1500, observer=None):
        ...
        with output or contextlib.nullcontext():
            ...
            if peak_index in (0, len(images) - 1):
                return failed(
                    "Der Verbindungspfad besitzt keine innere Energiebarriere."
                )
            ...
            if not self.search_converged:
                return failed(
                    "Die freie Sattelpunktverfeinerung ist nicht konvergiert."
                )
            ...
            for sign in (-1, 1):
                ...
                for end in references:
                    ...
                    delta = abs(
                        float(
                            downhill.get_potential_energy() - end.get_potential_energy()
                        )
                    )
                    matches.append(
                        {
                            ...
                            "matches": bool(
                                ...
                                and (
                                    np.cos(
                                        np.radians(downhill.get_dihedral(*self.indices))
                                    )
                                    > 0
                                )
                                == (
                                    np.cos(np.radians(end.get_dihedral(*self.indices)))
                                    > 0
                                )
                            ),
                        }
            ...
            connected = all(
                b["converged"] and b["bonds_preserved"] for b in branches
            ) and {b["isomer"] for b in branches} == {"cis", "trans"}
            self.connectivity = {
